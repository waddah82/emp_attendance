...
self.addEventListener("fetch", (event) => {
  ...
  // cached session (saved by app.js in Cache Storage)
  if (url.pathname === "/attendance/session.json") {
    event.respondWith(
      caches.open("emp-attendance-auth-v1")
        .then(c => c.match("/attendance/session.json"))
